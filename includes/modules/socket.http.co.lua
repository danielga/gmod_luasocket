...
local function receive(sock, pattern, prefix)
    local result, msg, partial = sock:receive(pattern, prefix)

    if running() ~= nil then
        while result == nil and #partial == 0 and msg ~= "closed" do
            yield()
            result, msg, partial = sock:receive(pattern, prefix)
        end
    ...
    return base.setmetatable({
        ...
        __call = function()
            ...
            if size > 0 then
                ...
                local chunk, err, part = receive(size)
                if chunk then receive(sock) end
                return chunk, err
            else
...
function _M.open(host, port, create)
    ...
    h.try(c:settimeout(running() ~= nil and 0 or _M.TIMEOUT))
    h.try(connect(c, host, port or _M.PORT))
...
function metat.__index:receivestatusline()
    local status = self.try(receive(self.c, 5))
    -- identify HTTP/0.9 responses, which do not contain a status line
    -- this is just a heuristic, but is what the RFC recommends
    if status ~= "HTTP/" then return nil, status end
    -- otherwise proceed reading a status line
    status = self.try(receive(self.c, "*l", status))
